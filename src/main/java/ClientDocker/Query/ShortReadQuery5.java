    ...
    public void run() {
        int[] randArray = new int[5];
            for(int i = 0; i < randArray.length; i++){
                randArray[i] = ((int)(Math.random() * randArray.length)) + 12000;
            }
        for(int j=0; j<randArray.length; j++ ){  
            try {
                existsInlocalMessage = false;
                existsInlocalPerson = false;
                existsInCacheMessage = false;
                existsInCachePerson = false;
            ...
            find_person_id = "SELECT * FROM message WHERE id="+randArray[j]+" AND serverId="+this.localId;
            find_message_id_cache = "SELECT * FROM message_query5 WHERE id="+randArray[j]+" AND serverId="+this.localId;
                    ...
                    */
                    ...
                    try {
                        if(this.neighbors.size() > 0) {
                            ...
                            subquery = "SELECT * FROM message WHERE id="+randArray[j]+" AND serverId=";
                            ...
                            for(int i = 0; i < this.neighbors.size(); ++i) {
                                ...
                                Future<String> future = executor.submit(new gRPC(serverSubquery, 1, this.localId, this.neighbors.get(i), randArray[j], this.queryID, this.temp_message_table_name, this.idToChannel));
        ...
        {        
            ...
            try{
                Statement statement = connection.createStatement();
                statement.executeUpdate("UPDATE socialnetwork.message_query5 SET creationDate = '" + currentTime + "' WHERE id = " + randArray[j]);
            }catch(Exception e){
            ...
            if(personSet.next()) {
                ...
                if(personSetCache.next()){
                    ...
                    for(int i = 0; i < this.neighbors.size(); ++i) {
                        ...
                        Future<String> future = executor.submit(new gRPC(person_subquery, 2, this.localId, this.neighbors.get(i), randArray[j], this.queryID, this.temp_person_table_name, this.idToChannel));                    
